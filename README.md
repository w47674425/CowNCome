# 牛与来 · 牧场文字合成 PvP 塔防（Web 版）

> 牧场题材文字合成塔防小游戏。将《赵云与阿斗》的基础循环改造为动物牧场主题
> （敌方为各类牛），快速落地为可玩 Web 版本，纯 TypeScript + Canvas 2D 实现，
> **零引擎依赖**，后续可低成本迁移微信小游戏。
...
竖版 4×3 棋盘，以汉字为棋子，通过**召唤 → 合成 → 部署**构建动物防线，抵挡一波波牛群，保卫牛犊。

**核心循环**

```
召唤（消耗干草，成本递增）→ 动物伙伴进入备选槽 → 部署到棋盘 / 同类同级合成升级
→ 牛群沿 4 条道推进 → 我方单位自动攻击 → 牛犊血量归零则失败
```

**特色机制**

- **文字合成**：部署时拖放至同类型同等级单位可合成升级（如 犬×2 → 犬Lv2），收益倍增
- **棋盘解锁**：初始 6 格免费，其余格子点击消耗干草解锁（40 → 70 → 110 → 160 → 220 → 300），花费递增；铲子可免费扩格
- **备选槽（4 格）**：开局随机赠送 2~3 名 Lv.1 伙伴；槽满时无法召唤，需先部署腾位
- **波次预告**：顶部面板实时预告下一波牛群的类型、数量与出兵方向，可提前布局
- **敌人类型化**：牛 / 牦 / 羚 / 犀 四类牛，血量与移速倍率各异
- **确定性波次**：30 波固定波次计划，预告与实机 100% 一致，可研究套路
...
- [x] Web 版 MVP：召唤 / 合成 / 部署 / 战斗 / 波次完整闭环
- [x] 备选槽 + 顶部波次预告
- [x] 棋盘付费解锁（消耗递增）
- [x] 动物牧场主题（敌方各类牛）
- [x] AI 对战冒烟测试（平衡接近五五开）
